...
float viewAngle = -160.0;	//Camera view angle
int N = 18;			// Number of verticies of polygon
...
{ 
    ...
    float texCoord[19] = { 0, 0.15, 0.2, 0.25, 0.3, 0.35 , 0.4, 0.433, 0.466, 0.5, 0.533, 0.566, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 1};
...
// =============================================================================
	...
	for (int block = 0; block < 9; block++) {
		for (int i = 0; i < N; i ++) {		//rotating the quads
			wx[i] = vx[i]*cos(theta* CDR) + vz[i]*sin(theta*CDR);
			wy[i] = vy[i] + 20;
			wz[i] = -vx[i]*sin(theta*CDR) + vz[i]*cos(theta*CDR);
			
			if (i > 0) normal(wx[i-1], wy[i-1], wz[i-1],	//joining the quads
							  vx[i], vy[i], vz[i],
							  wx[i], wy[i], wz[i]);
			glTexCoord2f(texCoord[i],0); glVertex3f(vx[i], vy[i], vz[i]);
			glTexCoord2f(texCoord[i],1); glVertex3f(wx[i], wy[i], wz[i]);	
		}
		glTexCoord2f(0,0); glVertex3f(vx[0], vy[0], vz[0]);	//mapping the texture
		glTexCoord2f(0,1); glVertex3f(wx[0], wy[0], wz[0]);
		for (int i = 0; i < N; i++) {	//replacing the values with transformed values
			vx[i] = wx[i];
			vy[i] = wy[i];
			vz[i] = wz[i];
		}
